...
|--------------------|-----------------|----|----------------------|----------------------------------|---------------------------------|
| 商户分账单号             | bizAllocationNo | 是  | String               | 123                              | 支付时客户系统传输的分账单号，需要保证唯一，可以为字符文字组合 |
| 支付订单号              | orderNo         | 否  | String               | P123                             |                                 |
| 商户支付订单号            | bizOrderNo      | 否  | String               | DEVP123                          |                                 |
| 分账描述               | description     | 否  | String               | 测试支付的描述                          |                                 |
| 分账组编号              | groupNo         | 否  | String               | 123                              | 优先级 分账接收方列表 > 分账组编号 > 默认分账组     |
...
| 字段名<img width=70/> | 变量名        | 必填 | 类型      | 示例值 | 描述<img width=200/> |
|--------------------|------------|----|---------|-----|--------------------|
| 分账接收方编号            | receiverNo | 是  | String  | 123 |                    |
...
|--------------------|-----------------|----|---------|----------------------------------|---------------------------------------------------------------------------------------|
| 商户订单号              | allocationNo    | 是  | String  | 1753370980523384832              | 发起支付时传输的商户订单号                                                                         |
| 订单号                | bizAllocationNo | 是  | String  | 1753370980523384832                             | 支付系统生成的支付订单号                                                                          |
